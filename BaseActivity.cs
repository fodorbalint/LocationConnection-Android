...
namespace LocationConnection
{
	public class BaseActivity : AppCompatActivity
	{
		...
		private static IFusedLocationProviderClient fusedLocationProviderClient;
		...
		protected async void StartLocationUpdates() //must run on UI thread
		{
            try
            {
				...
				{
                    locationRequest = new LocationRequest.Builder(Priority.PriorityBalancedPowerAccuracy, interval * 1000).SetMinUpdateIntervalMillis((long)(interval * 1000 * 0.8)).Build();
				}
				else
				{
                    locationRequest = new LocationRequest.Builder(Priority.PriorityHighAccuracy, interval * 1000).SetMinUpdateIntervalMillis((long)(interval * 1000 * 0.8)).Build();
                }
