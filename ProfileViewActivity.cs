...
namespace LocationConnection
{
	...
	public class ProfileViewActivity : BaseActivity, IOnMapReadyCallback, TouchActivity
	{
		...
		protected override void OnCreate(Bundle savedInstanceState)
        {
			try {
				...
				if (Settings.DisplaySize == 1 || Settings.DisplaySize is null)
				{
					...
					counterCircle = Resource.Drawable.counter_circle_normal;
					counterCircleSelected = Resource.Drawable.counter_circle_selected_normal;
				...
				else
				{
					...
					counterCircle = Resource.Drawable.counter_circle_small;
					counterCircleSelected = Resource.Drawable.counter_circle_selected_small;
		...
		public void OnMapReady(GoogleMap map) //will be called later than LoadUser?
		{
			...
			{
				MapStreet.SetBackgroundResource(Resource.Drawable.maptype_active_left);
				MapSatellite.SetBackgroundResource(Resource.Drawable.maptype_passive_right);
			}
			else
			{
				MapStreet.SetBackgroundResource(Resource.Drawable.maptype_passive_left);
				MapSatellite.SetBackgroundResource(Resource.Drawable.maptype_active_right);
			}
